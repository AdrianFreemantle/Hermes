...
namespace Hermes.Messaging
{
    public interface IPersistProcessManagers
    {
        ...
        void Create<T>(T state) where T : ProcessManagerData, new();

        void Update<T>(T state) where T : ProcessManagerData, new();
        ...
        /// <returns></returns>
        T Get<T>(Guid processId) where T : ProcessManagerData, new();

        /// <summary>
        /// Sets a saga as completed and removes it from the active saga list
        /// in the persistence store.
        /// </summary>
        void Complete<T>(Guid processId) where T : ProcessManagerData, new();

        T Find<T>(Expression<Func<T, bool>> expression) where T : ProcessManagerData, new();
    }
}